...
public class JBaahMain extends javax.swing.JFrame {
    ...
    private void SetBaseResponse() {
        ...
        for (int z = 0; z < hda.length; z++) {
            if ((!hda[z].toLowerCase(Locale.ENGLISH).startsWith("content-length:"))
                    && (!hda[z].toLowerCase(Locale.ENGLISH).startsWith("accept-encoding:"))
                    && (!hda[z].toLowerCase(Locale.ENGLISH).startsWith("connection:"))) {
                newReq += hda[z] + "\n";
            }
        }
